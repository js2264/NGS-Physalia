...
Scc1 is a key subunit of the cohesin complex, involved in the formation of chromatin 
loops and the maintenance of sister chromatid cohesion during cell division. 
...
tracks <- list(
    RNA_fwd = import('Share/day5/RNAseq.fwd.bw', as = "Rle"), 
    RNA_rev = import('Share/day5/RNAseq.rev.bw', as = "Rle"),
    Scc1 = import('Share/day5/Scc1.bw', as = "Rle")
)
...
bedtools getfasta \
    -name
    -fi Share/genome/R64-1-1.fa \
    -bed Scc1_peaks.narrowPeak \
    -fo scc1_peaks.fasta
xstreme \
    --order 2 \
    --oc streme_out \
...
- Comment these results. Do you think Scc1 is recruited to chromatin through specific DNA motifs?
...
gi_with_scc1 <- GInteractions(
    anchor1 = combn(1:length(scc1_peaks), 2)[1, ], 
    anchor2 = combn(1:length(scc1_peaks), 2)[2, ], 
    regions = scc1_peaks
...
g1_aggr <- aggregate(hic, targets = gi_with_scc1, flankingBins = 15)
hic <- import(cfs[['G2M']], resolution = 1000)
g2m_aggr <- aggregate(hic, targets = gi_with_scc1, flankingBins = 15)
cowplot::plot_grid(
    plotMatrix(g1_aggr, use.scores = 'detrended', scale = 'linear', limits = c(-0.25, 0.25), cmap = bwrColors()) + ggtitle("G1, Scc1 pairs"), 
    plotMatrix(g2m_aggr, use.scores = 'detrended', scale = 'linear', limits = c(-0.25, 0.25), cmap = bwrColors()) + ggtitle("G2M, Scc1 pairs")
)
```
:::
