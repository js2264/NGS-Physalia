...
library(rtracklayer)
library(tidyverse)
bedfiles <- list.files('Share/modENCODE/', pattern = '*bed')
peaks <- map(bedfiles, ~ import(file.path('Share/modENCODE/', .x)))
...
```{r, filename = 'R'}
library(readxl)
REs <- read_xlsx('Share/Celegans/WBcel235_REs.xlsx', skip = 2, col_names = TRUE)
