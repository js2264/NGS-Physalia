...
- Use `pairtools parse` to convert the `sam` file into a `pairs` file. Use the `--drop-seq` and `--drop-sam` options to lighten the output file. Also use `--output-stats hic.stats` to generate a summary of the parsing.
- Check the content of the `pairs` file with `head`.
- How many pairs are there in the `pairs` file?
- Check the content of the `hic.stats` file. What is the average mapping quality of the pairs?

#::: {.callout-note icon='true'}

`pairtools` has been installed in a separate `micromamba` environment `pairtools_env`, because it is incompatible with other tools we will use in the workshop. 
...
::: {.callout-answer .icon .callout-note collapse=true}
```{sh, filename = 'bash'}
micromamba run -n pairtools_env pairtools parse \
...
::: {.callout-answer .icon .callout-note collapse=true}
```{sh, filename = 'bash'}
micromamba run -n pairtools_env pairtools sort --nproc 2 -o HiC.sorted.pairs HiC.pairs

micromamba run -n pairtools_env pairtools dedup \
    --max-mismatch 3 \
    --mark-dups \
    --output HiC.sorted.dedup.pairs \
    HiC.sorted.pairs

micromamba run -n pairtools_env pairtools select 'pair_type=="UU"' HiC.sorted.dedup.pairs -o HiC_filtered.pairs
```
:::
...
::: {.callout-answer .icon .callout-note collapse=true}
```{sh, filename = 'bash'}
cooler tree ~/Share/HiC/HiC.cool
cooler info ~/Share/HiC/HiC.cool
```
:::
